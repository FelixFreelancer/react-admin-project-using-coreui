...
class User extends Component {
	...
	componentDidMount() {
		...
		axios.get(Constants.BASE_URL + 'api/users/' + userId + '/' + token)
			.then(res => {
	...
	handleUpdatePassword = () => {
		const {update_user_info} = this.state
		axios.post(Constants.BASE_URL + 'api/users/update-password', {
	...
	handleUpdateEmail = () => {
		...
		axios.post(Constants.BASE_URL + 'api/users/update-email', {
					...
					<Row>
						<Col xs={"12"} sm={"6"}>
							<FormGroup check style={{margin: '20px'}}>
								...
								<Input onClick={this.handleToggleChangePassword}
								       className="form-check-input"
								       type="checkbox" id="changePassword" name="changePassword"/>
								<Label check className="form-check-label" htmlFor="changePassword">Change
									Password</Label>
									...
									<Col>
										...
										</FormGroup>
										{
											this.state.old_password_validation?
												<p style={{color:'red'}}>Your password is not correct. </p>:
												''
										}
										<FormGroup row>
											<Col xs="4">
						...
						<Col xs={"12"} sm={"6"}>
							<FormGroup check style={{margin: '20px'}}>
								...
								<Input className="form-check-input"
								       ...
								       type="checkbox" id="changeEmail" name="changeEmail"
								       value="true"/>
								<Label check className="form-check-label" htmlFor="changeEmail">Change Email</Label>
										...
										<FormGroup row>
											...
											<Col xs="8">
												<Input type="text"
												       ...
												       disabled
												       id="dishDiscount" name="dishDiscount"/>
											</Col>
										</FormGroup>
										<FormGroup row>
											<Col xs="4">
												<Label htmlFor="dishTotalPrice">
													<strong
														style={{color: this.state.email_validation !== '' ? 'red' : 'black'}}>
														New Email
													</strong>
												</Label>
											</Col>
											<Col xs="8">
												<Input type="email"
												       style={{
													       border: this.state.email_validation !== '' ? '1px solid red' : '',
